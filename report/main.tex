...
matrix, is also symmetric, hence it is tridiagonal—from now we will refer to it as $ T_j $—hence
...
- I \|_2 > \varepsilon $ for a chosen $ \varepsilon = 10^{-2} $. When the algorithm is called
passing a given threshold $ \varepsilon_{\text{FOM}} $, it computes the residual $
\|r_j^{\text{FOM}} \|_2  \mid \beta_j  \mid  \mid e_j^{\top}y_j^{\text{FOM}}$ solving the linear
system $ T_jy_j^{\text{FOM}} = \|b\|_2 e_1 $ numerically calling Numpy's function \texttt{solve($ T_j $,$\|b\|_2$)}.
...
\begin{algorithm}[ht!]
  ...
  \begin{algorithmic}
    ...
    \State $\left\{
    \begin{array}{ll}
        w \leftarrow w - \alpha_j &  \text{No reorthogonalization} \\
        \text{\textbf{if} } j>0 \text{ \textbf{then} } w \leftarrow w - \beta_j \text{ \textbf{end
    if}} & \\ \\
    ...
    \If{$ \beta_j = 0 $}
      ...
      \Comment{--- Breakdown ---}
    \Else
      \If{$ \|r_j^{\text{FOM}}\|_2 =  \mid b_j\mid  \mid e_j^{\top}y_j^{\text{FOM}} \mid <
      \varepsilon_{\text{FOM}}$}
      \State 
      \Return $ V_j(:,1:j), \alpha_j(1:j), \beta_j(1:j-1) $
