...
class Ex23:
    """Run experiments 2 and 3 and persist results under ``./out``.

    This class generates (or loads) cached Erdős-Rényi graphs, runs Lanczos on
    them with a stopping tolerance, and writes LaTeX tables plus serialized
    DataFrames.
    """
    ...
    def run(self) -> tuple[pd.DataFrame, pd.DataFrame]:
        """Run Experiment 2 and 3 and save outputs.

        Returns:
            A tuple ``(df2, df3)`` with the per-run results for Experiment 2 and 3.
        """
    ...
    def _experiment_params(self) -> tuple[np.ndarray, float, int, np.ndarray]:
        """Return the fixed parameter grids used by experiments 2 and 3."""
        p_ex2 = 0.04
        M = 200
    ...
    def _graph_key(self, N: int, p: float, t_index: int) -> tuple[int, float, int]:
        """Build a stable key for graph caching."""
        return (int(N), float(p), int(t_index))

    def _generate_graphs(self) -> None:
        """Generate and serialize graphs if cache is missing."""
    ...
    def _get_graph(self, N: int, p: float, t_index: int) -> Any:
        """Recreate a :class:`pygsp.graphs.Graph` from cached adjacency."""
    ...
    def _perform_iteration(
        ...
        Args:
            ...
            t_index: Repetition index (used to select the cached graph).
        ...
        _, _, debug = lanczos(L, s, M, g=g, tau=tau)
    ...
    def print_group_table(self):
        """Print grouped summary tables for Experiment 2 and 3."""
